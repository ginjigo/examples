...
func listUsers(c *ginji.Context) error {
	return c.JSON(200, users)
}

func getUser(c *ginji.Context) error {
	id := c.Param("id")
...
func createUser(c *ginji.Context) error {
...
func updateUser(c *ginji.Context) error {
	id := c.Param("id")
	var req CreateUserRequest
	if err := c.BindJSON(&req); err != nil {
		return c.JSON(400, ErrorResponse{
...
func deleteUser(c *ginji.Context) error {
	id := c.Param("id")
	for i, user := range users {
		if user.ID == parseID(id) {
