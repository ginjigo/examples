...
func main() {
	...
	typed.Get("/users/:id", func(c *ginji.Context, req GetUserRequest) (User, error) {
		...
		if _, err := fmt.Sscanf(req.ID, "%d", &id); err != nil {
			return User{}, ginji.NewHTTPError(ginji.StatusBadRequest, "Invalid ID format")
		}
	...
	v1.Put("/users/:id", func(c *ginji.Context) error {
		...
		if _, err := fmt.Sscanf(id, "%d", &userID); err != nil {
			c.AbortWithError(ginji.StatusBadRequest, fmt.Errorf("invalid ID format: %w", err))
			return nil
		}
	...
	typed.Delete("/users/:id", func(c *ginji.Context, req GetUserRequest) (ginji.EmptyRequest, error) {
		id := 0
		if _, err := fmt.Sscanf(req.ID, "%d", &id); err != nil {
			return ginji.EmptyRequest{}, ginji.NewHTTPError(ginji.StatusBadRequest, "Invalid ID format")
		}
